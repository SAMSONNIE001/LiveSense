...
function renderReport(report) {
  ...
  const overallLevel = current.alarm_active || current.phone_at_ear || current.face_missing_warning ? "danger" : ["Dozing", "Drowsy"].includes(current.sleep_state) || current.eating_detected || current.drinking_detected ? "warning" : "";
  const overallTitle = current.alarm_active ? "Critical sleep danger detected" : current.phone_at_ear ? "Unsafe phone use detected" : overallLevel === "warning" ? "Safety attention is required" : current.face_detected ? "Monitoring signals are normal" : "Waiting for clear camera signals";
  ...
  const observations = [
    ...
    observation("Phone use", current.phone_at_ear ? "Detected" : "Clear", current.phone_at_ear ? "danger" : "good"),
