...
function updateActivityRecord(c) {
  const records = [
    ...
    ["Yawning", c.yawning ? "Detected" : "Clear", c.yawning ? "warning" : "active"],
    ["Attention", `${Math.round(c.attention)}%`, c.attention >= 60 ? "active" : c.attention >= 35 ? "warning" : "danger"],
    ["Readiness", `${Math.round(c.readiness)}%`, c.readiness >= 60 ? "active" : c.readiness >= 35 ? "warning" : "danger"],
  ];
  $("#activity-record").innerHTML = records.map(([name, value, state]) => `<div class="record-row"><i class="${state}"></i><span>${name}</span><b class="${state}">${value}</b></div>`).join("");
}
...
function drawChart(canvas, history, series) {
  ...
  for(const [key,color] of series){ctx.strokeStyle=color;ctx.lineWidth=1.5;ctx.beginPath();history.forEach((p,i)=>{const x=history.length<2?0:i*w/(history.length-1),value=key==="yawning"?(p[key]?100:0):key==="sleeping"?(p.alarm_active||p.sleep_state==="Sleeping"?100:0):(p[key]||0),y=h-8-Math.max(0,Math.min(100,value))*(h-16)/100;i?ctx.lineTo(x,y):ctx.moveTo(x,y)});ctx.stroke();}
}
